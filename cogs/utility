...
async def setup(bot):
    await bot.add_cog(Utility(bot))
      ...
      async def on_member_join(self, member):
          async with aiosqlite.connect(DB_PATH) as db:
              async with db.execute("SELECT welcome_channel, welcome_message FROM guild_settings WHERE guild_id = ?",
  (member.guild.id,)) as cur:
          ...
          if not ch:
              return
          msg = (row[1] or "Welcome {user} to {server}!").replace("{user}", member.mention).replace("{server}",
  member.guild.name)
          embed = discord.Embed(description=msg, color=0x7c3aed)
          embed.set_thumbnail(url=member.display_avatar.url)
          await ch.send(embed=embed)
      ...
      async def on_member_remove(self, member):
          async with aiosqlite.connect(DB_PATH) as db:
              async with db.execute("SELECT goodbye_channel, goodbye_message FROM guild_settings WHERE guild_id = ?",
  (member.guild.id,)) as cur:
                  row = await cur.fetchone()
          if not row or not row[0]:
              return
          ...
          if not ch:
              return
          msg = (row[1] or "{user} has left {server}.").replace("{user}", str(member)).replace("{server}",
  member.guild.name)
          await ch.send(embed=discord.Embed(description=msg, color=0xff4444))
      ...
      async def on_reaction_add(self, reaction, user):
          ...
          guild_id = reaction.message.guild.id
          async with aiosqlite.connect(DB_PATH) as db:
              async with db.execute("SELECT starboard_channel, starboard_threshold FROM guild_settings WHERE guild_id =
  ?", (guild_id,)) as cur:
                  row = await cur.fetchone()
          if not row or not row[0]:
              return
          ...
          async with aiosqlite.connect(DB_PATH) as db:
              async with db.execute("SELECT star_message_id FROM starboard_posts WHERE guild_id = ? AND message_id = ?",
  (guild_id, reaction.message.id)) as cur:
                  if await cur.fetchone():
                      return
  ...
  icon_url=reaction.message.author.display_avatar.url)
          ...
          async with aiosqlite.connect(DB_PATH) as db:
              await db.execute("INSERT INTO starboard_posts VALUES (?, ?, ?)", (guild_id, reaction.message.id,
  star_msg.id))
              await db.commit()
      ...
      async def userinfo(self, interaction: discord.Interaction, member: discord.Member = None):
          ...
          embed.add_field(name="ID", value=str(m.id))
          embed.add_field(name="Nickname", value=m.nick or "None")
          embed.add_field(name="Joined", value=discord.utils.format_dt(m.joined_at, style="D") if m.joined_at else
  "Unknown")
          embed.add_field(name="Created", value=discord.utils.format_dt(m.created_at, style="D"))
          roles = [r.mention for r in m.roles[1:]]
          embed.add_field(name=f"Roles ({len(roles)})", value=", ".join(roles) or "None", inline=False)
      ...
      async def setup(self, interaction: discord.Interaction,
          ...
          async with aiosqlite.connect(DB_PATH) as db:
              await db.execute("""
                  INSERT INTO guild_settings (guild_id, log_channel, welcome_channel, goodbye_channel,
  starboard_channel, level_up_channel)
                  VALUES (?, ?, ?, ?, ?, ?)
                  ON CONFLICT(guild_id) DO UPDATE SET
                      log_channel = COALESCE(?, log_channel),
                      welcome_channel = COALESCE(?, welcome_channel),
          ...
          embed = discord.Embed(title="Nightshade Setup Updated", color=0x7c3aed)
          for name, ch in [("Log", log_channel), ("Welcome", welcome_channel), ("Goodbye", goodbye_channel),
  ("Starboard", starboard_channel), ("Level-Up", level_up_channel)]:
              if ch:
                  embed.add_field(name=f"{name} Channel", value=ch.mention)
          await interaction.response.send_message(embed=embed)
      ...
      async def ticket(self, interaction: discord.Interaction, reason: str = "No reason provided"):
          try:
              ...
              await thread.add_user(interaction.user)
              embed = discord.Embed(title="Support Ticket", color=0x7c3aed,
                                    description=f"Hello {interaction.user.mention}! Staff will be with you
  shortly.\n\n**Reason:** {reason}")
              await thread.send(embed=embed)
              await interaction.response.send_message(f"Ticket created: {thread.mention}", ephemeral=True)
          except discord.Forbidden:
              await interaction.response.send_message("I don't have permission to create private threads here.",
  ephemeral=True)
