...
async def setup(bot):
  ...
  class Economy(commands.Cog):
      ...
      async def get_balance(self, guild_id, user_id):
          async with aiosqlite.connect(DB_PATH) as db:
              async with db.execute("SELECT coins FROM economy WHERE guild_id = ? AND user_id = ?", (guild_id, user_id))
  as cur:
                  row = await cur.fetchone()
      ...
      async def update_balance(self, guild_id, user_id, amount):
          async with aiosqlite.connect(DB_PATH) as db:
              await db.execute(
                  "INSERT INTO economy (guild_id, user_id, coins) VALUES (?, ?, 0) ON CONFLICT(guild_id, user_id) DO
  NOTHING",
                  (guild_id, user_id)
              )
              await db.execute(
      ...
      async def daily(self, interaction: discord.Interaction):
          ...
          if row and row[0] == today:
              return await interaction.response.send_message("Already claimed today. Come back tomorrow!",
  ephemeral=True)

          amount = random.randint(500, 1000)
          async with aiosqlite.connect(DB_PATH) as db:
              await db.execute(
                  "INSERT INTO economy (guild_id, user_id, coins, last_daily) VALUES (?, ?, ?, ?) ON CONFLICT(guild_id,
  user_id) DO UPDATE SET coins = coins + ?, last_daily = ?",
                  (interaction.guild_id, interaction.user.id, amount, today, amount, today)
              )
              await db.commit()

          embed = discord.Embed(title="Daily Reward!", description=f"You received **🪙 {amount:,} coins**!",
  color=0xffd700)
      ...
      async def gamble(self, interaction: discord.Interaction, amount: int):
          ...
          if random.random() < 0.45:
              ...
              embed = discord.Embed(title="You Won!", description=f"You won **🪙 {winnings:,}** (net +{net:,})!",
  color=0x00ff00)
          else:
